...
本系统的 HTTP 数据源监控功能允许用户自定义 HTTP 请求地址，系统会在服务端主动发起 HTTP 请求并获取响应结果，再通过自定义表达式判断是否触发报警。此为核心功能设计，SSRF（服务端请求伪造）漏洞由此不可避免地引入。

> **本系统设计定位为内网运维工具，强烈建议将本系统仅部署于企业内网，切勿对公网暴露，可有效避免恶意 SSRF 攻击**
