...
本系统的 HTTP 数据源监控功能允许用户自定义 HTTP 请求地址，系统会在服务端主动发起 HTTP 请求并获取响应结果，再通过自定义表达式判断是否触发报警。此功能设计上具有SSRF（服务端请求伪造）的潜在风险。

>本系统设计定位为内网运维工具，强烈建议将本系统仅部署于企业内网，切勿对公网暴露，这是最有效的安全基础。

**请注意与建议**

1. 内网部署是基础，但非绝对安全：请知晓，即使部署于内网，该功能仍可能被恶意用户用于访问或探测内网其他服务。在云服务器上部署时，需特别注意配置，防止访问云平台元数据服务。
1. 遵循最小权限原则：在配置监控地址时，应尽可能精确、固定，避免使用可由用户随意构造的动态URL。
1. 建议的防护方向：您可以通过配置网络防火墙规则、使用代理网关、或对部署主机进行网络层访问控制，来约束本服务的出站请求范围，以进一步提升安全性。

**最后提醒**：安全是一个持续的过程，而非一次性任务，我们也将在后续提供进一步的安全加固。请根据您的安全要求评估风险，并实施适当的安全控制措施。
